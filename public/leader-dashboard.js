...
const itemCodes = Object.keys(inventory);
...
async function loadRoleIntro(roleNameToLoad) { 
    ...
    try {
        ...
        const messagesResponse = await fetch(`/text/elder-chew-${roleNameToLoad.toLowerCase()}.json`);
...
const N = 5;
const PLAYER = '🧑‍🚀';
const ITEMS = ['S','F','W','M']; // S=✨, F=🪶, W=🪵, M=🔩
const ITEM_EMOJI = { S: '✨', F: '🪶', W: '🪵', M: '🔩' };
const MAX_STEPS = 8;
const total = N * N;
let itemList = [];
