...
// Serve role-select.html specifically for the root path
app.get('/', (req, res) => {
...
function createSession(id) {
    return {
        ...
        roles: {},
        started: false,
        state: { inventory: { W: 0, M: 0, F: 0, S: 0 } },
        log: [],
        reflections: {},
...
wss.on('connection', function connection(ws) {
    ...
    ws.on('message', function incoming(message) {
        ...
        } else if (!ws.playerId && type === 'session-start' && payload.role === 'Leader') {
       ...
       if (type === 'move' || type === 'inject' || type === 'inventory-update') {
        ...
        if (type === 'member-clicked') {
            ...
            const leaderId = Object.entries(session.roles).find(([role, id]) => role === 'Leader')?.[1];
